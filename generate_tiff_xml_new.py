...
def get_cell_image(path, ctype, parent_pathes):
    ...
    for item in files:
        if item.endswith('.jpg'):
            ...
            # 细胞所属大图名称
            pattern = re.compile(r'1\-p0\.\d{10}_(.*?)_x(\d+)_y(\d+)_w(\d+)_h(\d+)_?(\dx)?.jpg')
...
if __name__ == '__main__':
    ...
    # 自动标注细胞图像存储位置
    auto_path = [
        '/home/cnn/Development/code/Pathological_Cell_Tracker/CELL_CLASSIFIED_JOB_20181106_NORMAL/CELLS',
    ]
